...
def touchingBadConvexSets (p : ℝ) (chosen : Set (Set Plane)) (C : Set Plane) :
    Set (Set Plane) :=
...
theorem subset_ball_two_mul_of_diameterThickening_three_inter {V C : Set Plane}
    (hV : IsBounded V) {z : Plane} {rho : ℝ}
...
theorem mul_tsum_ediam_touchingBadConvexSets_le {mu : MeasureTheory.Measure Plane}
    ...
    (hz : ∀ V : chosen, z ∉ diameterThickening 7 (V : Set Plane))
    (hC : C ⊆ Metric.closedBall z rho) :
    ENNReal.ofReal alpha *
        ∑' V : touchingBadConvexSets 3 chosen C, Metric.ediam (V : Set Plane) ≤
...
theorem tsum_ediam_touchingBadConvexSets_lt_ediam {mu : MeasureTheory.Measure Plane}
    ...
    {C : Set Plane} {z : Plane} {rho : ℝ} (hrho : 0 < rho)
    (hz : ∀ V : chosen, z ∉ diameterThickening 7 (V : Set Plane))
    (hC : C ⊆ Metric.closedBall z rho)
    ...
    ∑' V : touchingBadConvexSets 3 chosen C,
        Metric.ediam (diameterThickening 3 (V : Set Plane)) < Metric.ediam C := by
  ...
  have hsum : (∑' V : touchingBadConvexSets 3 chosen C,
      Metric.ediam (V : Set Plane)) < ENNReal.ofReal (sigma * rho / 14) := by
    exact lt_of_mul_lt_mul_left (hpacking.trans_lt houtside) (by positivity)
  calc
    (∑' V : touchingBadConvexSets 3 chosen C,
        Metric.ediam (diameterThickening 3 (V : Set Plane))) ≤
        ∑' V : touchingBadConvexSets 3 chosen C,
          ENNReal.ofReal 7 * Metric.ediam (V : Set Plane) :=
