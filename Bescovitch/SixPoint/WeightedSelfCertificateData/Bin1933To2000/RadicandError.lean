...
/-!
# Radicand approximation error on `[1933/5000, 2/5]`
-/
...
private theorem radicand_error_lt : approxFormula.radicand.error < 7 / 10 ^ 12 := by
  with_unfolding_all rfl

set_option maxRecDepth 100000 in
set_option maxHeartbeats 0 in
private theorem radicand_nominal_abs_bound :
    absBound approxFormula.radicand.nominal ≤ 29 / 500 := by
  with_unfolding_all rfl
...
/-- The radicands obey the stated error and nominal-magnitude bounds. -/
theorem radicand_error_and_nominal_magnitude (x y z : I) :
    let exactRadicand := (weightedSelfRealFormula
        (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).r
        (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).b
        (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).t (2 / 5)).radicand
    |exactRadicand - formula.radicand.eval x y z| < (7 / 10 ^ 12 : ℝ) ∧
      |formula.radicand.eval x y z| ≤ (29 / 500 : ℝ) := by
  have hrad := (approx_formula_rel x y z).2.2
  have herror := hrad.2.2
  have hstrict : (approxFormula.radicand.error : ℝ) < (7 / 10 ^ 12 : ℝ) := by
    have hconstant : ((7 / 10 ^ 12 : ℚ) : ℝ) = (7 / 10 ^ 12 : ℝ) := by
      norm_num
    rw [← hconstant]
    exact (Rat.cast_lt (K := ℝ)).2 radicand_error_lt
  have hrNominal := abs_le_abs_bound hrad.2.1
  dsimp only
  refine ⟨herror.trans_lt hstrict, ?_⟩
  calc
    _ ≤ ((absBound approxFormula.radicand.nominal : ℚ) : ℝ) := hrNominal
    _ ≤ ((29 / 500 : ℚ) : ℝ) :=
      (Rat.cast_le (K := ℝ)).2 radicand_nominal_abs_bound
    _ = 29 / 500 := by norm_num
