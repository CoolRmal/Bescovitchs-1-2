...
/-!
# Magnitude bounds for P on `[1933/5000, 2/5]`
-/
...
private theorem p_exact_abs_bound :
    absBound approxFormula.p.exact ≤ 483 / 100 := by
  with_unfolding_all rfl

set_option maxRecDepth 100000 in
set_option maxHeartbeats 0 in
private theorem p_nominal_abs_bound :
    absBound approxFormula.p.nominal ≤ 483 / 100 := by
  with_unfolding_all rfl
...
/-- The exact and nominal values of `P` have magnitude at most `483/100`. -/
theorem p_magnitude_bounds (x y z : I) :
    let exactData := weightedSelfRealFormula
      (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).r
      (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).b
      (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).t (2 / 5)
    |exactData.p| ≤ (483 / 100 : ℝ) ∧
      |formula.p.eval x y z| ≤ (483 / 100 : ℝ) := by
  obtain ⟨hp, _, _⟩ := approx_formula_rel x y z
  have hpExact := abs_le_abs_bound hp.1
  have hpNominal := abs_le_abs_bound hp.2.1
  dsimp only
  constructor
  · calc
      _ ≤ ((absBound approxFormula.p.exact : ℚ) : ℝ) := hpExact
      _ ≤ ((483 / 100 : ℚ) : ℝ) :=
        (Rat.cast_le (K := ℝ)).2 p_exact_abs_bound
      _ = 483 / 100 := by norm_num
  · calc
      _ ≤ ((absBound approxFormula.p.nominal : ℚ) : ℝ) := hpNominal
      _ ≤ ((483 / 100 : ℚ) : ℝ) :=
        (Rat.cast_le (K := ℝ)).2 p_nominal_abs_bound
      _ = 483 / 100 := by norm_num
