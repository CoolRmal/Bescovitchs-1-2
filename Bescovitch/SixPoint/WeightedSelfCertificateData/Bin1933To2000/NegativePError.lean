...
/-!
# Approximation error for P on `[1933/5000, 2/5]`
-/
...
private theorem p_error_lt : approxFormula.p.error < 7 / 10 ^ 10 := by
...
/-- The exact and dyadic values of `P` differ by less than `7 / 10¹⁰`. -/
theorem exact_p_error (x y z : I) :
    |(weightedSelfRealFormula
        (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).r
        (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).b
        (weightedSelfRealChart (1933 / 5000) (2 / 5) x y z).t (2 / 5)).p -
      formula.p.eval x y z| < (7 / 10 ^ 10 : ℝ) := by
  have herror := (approx_formula_rel x y z).1.2.2
  have hstrict : (approxFormula.p.error : ℝ) < (7 / 10 ^ 10 : ℝ) := by
    have hconstant : ((7 / 10 ^ 10 : ℚ) : ℝ) = (7 / 10 ^ 10 : ℝ) := by
      norm_num
    rw [← hconstant]
    exact (Rat.cast_lt (K := ℝ)).2 p_error_lt
