...
theorem exists_oriented_weightedPairScore
    ...
    ∃ orientation : ℝ, (orientation = 1 ∨ orientation = -1) ∧
      ...
      weightedPairScore (orientedCoordinates e orientation e) barC endpointLambda endpointMu
        ...
        weightedPairScore e barC endpointLambda endpointMu p₁ p₂ w₁ w₂ := by
  ...
  simpa [T] using
    weightedPairScore_linearIsometry T e barC endpointLambda endpointMu p₁ p₂ w₁ w₂
...
theorem exists_weightedPairScore_lensChart (e p₁ p₂ w₁ w₂ : (EuclideanSpace ℝ (Fin 2)))
    ...
    (hw₁ : ‖w₁‖ ≤ 1) (hw₂ : ‖w₂‖ ≤ 1)
    (hpChord : ‖p₁ - p₂‖ = barC) (hwChord : ‖w₁ - w₂‖ = barC) :
    ∃ sideP zP aP hP sideW zW aW hW : ℝ,
      (sideP = 1 ∨ sideP = -1) ∧ (sideW = 1 ∨ sideW = -1) ∧
      0 ≤ zP ∧ zP ≤ 1 ∧ -1 ≤ zW ∧ zW ≤ 1 ∧
      aP ^ 2 + hP ^ 2 ≤ 1 ∧ (aP - barC) ^ 2 + hP ^ 2 ≤ 1 ∧
      aW ^ 2 + hW ^ 2 ≤ 1 ∧ (aW - barC) ^ 2 + hW ^ 2 ≤ 1 ∧
  ...
  have hPFirst : aP ^ 2 + hP ^ 2 ≤ 1 := by
    ...
    exact hsquare
  have hPSecond : (aP - barC) ^ 2 + hP ^ 2 ≤ 1 := by
    have hsquare : ‖P₂‖ ^ 2 ≤ 1 := by nlinarith [norm_nonneg P₂]
    rw [hP₂, norm_chordChartSecond_sq barC aP hP zP hsidePSq] at hsquare
    exact hsquare
  have hWFirst : aW ^ 2 + hW ^ 2 ≤ 1 := by
    ...
    exact hsquare
  have hWSecond : (aW - barC) ^ 2 + hW ^ 2 ≤ 1 := by
    have hsquare : ‖W₂‖ ^ 2 ≤ 1 := by nlinarith [norm_nonneg W₂]
    rw [hW₂, norm_chordChartSecond_sq barC aW hW zW hsideWSq] at hsquare
    exact hsquare
...
def WeightedLensChartBound : Prop :=
  ∀ sideP zP aP hP sideW zW aW hW : ℝ,
    ...
    weightedPairScore !₂[1, 0] barC endpointLambda endpointMu
      (chordChartFirst sideP aP hP zP) (chordChartSecond sideP barC aP hP zP)
      (chordChartFirst sideW aW hW zW) (chordChartSecond sideW barC aW hW zW) ≤ 0
...
theorem weightedPairScore_nonpos_of_lensChartBound (hchart : WeightedLensChartBound)
    ...
    (hw₂ : ‖w₂‖ ≤ 1)
    (hpChord : ‖p₁ - p₂‖ = barC) (hwChord : ‖w₁ - w₂‖ = barC) :
    weightedPairScore e barC endpointLambda endpointMu p₁ p₂ w₁ w₂ ≤ 0 := by
...
/-- Radial chord reduction extends the lens-chart bound to sibling distances at least `barC`. -/
theorem weightedPairScore_nonpos_of_lensChartBound_of_separated
    (hchart : WeightedLensChartBound)
    (e p₁ p₂ w₁ w₂ : (EuclideanSpace ℝ (Fin 2))) (he : ‖e‖ = 1)
    (hp₁ : ‖p₁‖ ≤ 1) (hp₂ : ‖p₂‖ ≤ 1) (hw₁ : ‖w₁‖ ≤ 1)
    (hw₂ : ‖w₂‖ ≤ 1)
    (hpChord : barC ≤ ‖p₁ - p₂‖) (hwChord : barC ≤ ‖w₁ - w₂‖) :
    weightedPairScore e barC endpointLambda endpointMu p₁ p₂ w₁ w₂ ≤ 0 := by
  obtain ⟨p₂', w₂', hpChord', hwChord', hp₂'Norm, hw₂'Norm, hscore⟩ :=
    exists_weightedPairScore_chord_reduction e one_lt_barC_and_barC_lt_two.1 hp₁ hw₁
      hpChord hwChord endpointMu_pos.le barC_weight_reduction_margin
  apply hscore.trans
