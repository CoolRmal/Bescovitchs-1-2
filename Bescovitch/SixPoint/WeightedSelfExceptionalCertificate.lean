...
private theorem evalOn_contains {n : ℕ} {input : Fin n → ℝ}
  ...
  induction h with
  | nil => simpa [evalOn, RadicalUnivariate.eval] using
      RationalInterval.singleton_contains 0
  | cons hhead htail ih =>
      exact RationalInterval.add_contains hhead
...
def evalOn : IntervalBivariate → RationalInterval → RationalInterval →
    RationalInterval
  | [], _, _ => .singleton 0
...
private theorem evalOn_contains {n : ℕ} {input : Fin n → ℝ}
  ...
  induction h with
  | nil => simpa [evalOn, RadicalBivariate.eval] using
      RationalInterval.singleton_contains 0
  | cons hhead htail ih =>
...
private theorem evalOn_contains {n : ℕ} {input : Fin n → ℝ}
  ...
  induction h with
  | nil => simpa [evalOn, RadicalTrivariate.eval] using
      RationalInterval.singleton_contains 0
  | cons hhead htail ih =>
...
private theorem contains_leftHalf_or_rightHalf {I : RationalInterval} {x : ℝ}
  ...
  · right
    constructor
    · have hmid' : (((I.lower + I.upper) / 2 : ℚ) : ℝ) ≤ x :=
        le_of_not_ge hmid
      exact_mod_cast hmid'
    · exact hx.2
...
theorem RadicalTrivariate.nonneg_of_interval_box_certificate {n : ℕ}
  ...
  | leaf =>
      ...
      have hlowerReal : (0 : ℝ) ≤ (P.evalOn X Y Z).lower := by exact_mod_cast hlower
      exact hlowerReal.trans hvalue.1
