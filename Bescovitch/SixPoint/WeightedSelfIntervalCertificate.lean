...
/-!
# Interval certificates for the weighted-self radius bins

Above the sharp constant no radius bin carries an equality configuration, so every bin closes by
a plain interval certificate: an interval-Horner subdivision for `-P` and a Bernstein subdivision
for the discriminant.  The sign of `Q` is supplied once and for all by the direct interval
evaluation on the same bin.
-/
