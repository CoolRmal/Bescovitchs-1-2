...
private theorem offMatching_pair_le {E : Type*} [NormedAddCommGroup E]
    ...
    (hx₁ : ‖x₁‖ ≤ 1) (hx₂ : ‖x₂‖ ≤ 1)
    (hseparation : cStar ≤ ‖x₁ - x₂‖) :
    3003 / 400 * ‖x₁‖ ^ 2 + 231 / 64 * ‖x₂‖ ^ 2 -
        2 * ⟪e, (39 / 10 : ℝ) • x₁ + (3 / 2 : ℝ) • x₂⟫_ℝ -
        7 / 2 * (cStar - 1) * ‖x₁‖ - 7 / 2 * (cStar + 1) * ‖x₂‖ -
        15 / 16 * cStar ^ 2 ≤ 831 / 100 := by
  let z := (39 / 10 : ℝ) • x₁ + (3 / 2 : ℝ) • x₂
  let Q := 1053 / 50 * ‖x₁‖ ^ 2 + 81 / 10 * ‖x₂‖ ^ 2 -
    117 / 20 * cStar ^ 2
  have hseparationSq : cStar ^ 2 ≤ ‖x₁ - x₂‖ ^ 2 := by
    nlinarith [cStar_pos, norm_nonneg (x₁ - x₂)]
  ...
  have hvertices := separableQuadratic_le_radial_vertices
    ...
    (b₁ := -7 / 2 * (cStar - 1)) (b₂ := -7 / 2 * (cStar + 1))
    (d := 84 / 25 - 75 / 28 * cStar ^ 2) (c := cStar)
    (t₁ := ‖x₁‖) (t₂ := ‖x₂‖) (by norm_num) (by norm_num) hx₁ hx₂ hsum
  have hvertexBound :
      max (38571 / 2800 + (-7 / 2 * (cStar - 1)) + 2697 / 448 +
          (-7 / 2 * (cStar + 1)) + (84 / 25 - 75 / 28 * cStar ^ 2))
        (max (38571 / 2800 + (-7 / 2 * (cStar - 1)) +
            2697 / 448 * (cStar - 1) ^ 2 +
            (-7 / 2 * (cStar + 1)) * (cStar - 1) +
            (84 / 25 - 75 / 28 * cStar ^ 2))
          (38571 / 2800 * (cStar - 1) ^ 2 +
            (-7 / 2 * (cStar - 1)) * (cStar - 1) + 2697 / 448 +
            (-7 / 2 * (cStar + 1)) +
            (84 / 25 - 75 / 28 * cStar ^ 2))) ≤ 831 / 100 := by
...
theorem offMatchingCoincidentTangentCertificate {E : Type*} [NormedAddCommGroup E]
  ...
  have hpinner :
      2 * ⟪p₁, p₂⟫_ℝ ≤ ‖p₁‖ ^ 2 + ‖p₂‖ ^ 2 - cStar ^ 2 := by
    rw [norm_sub_sq_real] at hpsepSq
    nlinarith
  have hwinner :
      2 * ⟪w₁, w₂⟫_ℝ ≤ ‖w₁‖ ^ 2 + ‖w₂‖ ^ 2 - cStar ^ 2 := by
  ...
  have hconstant :
      27 / 5 + 7 * (35 / 12) + 12 - 14 + 33 * cStar - 45 * cStar ^ 2 +
        2 * (831 / 100) < 0 := by
    rcases cStar_mem_isolation_box with ⟨hlower, hupper⟩
    norm_num at hlower hupper ⊢
    nlinarith [sq_nonneg (cStar - 1)]
