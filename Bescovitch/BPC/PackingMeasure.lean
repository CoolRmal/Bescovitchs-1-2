...
public import Bescovitch.BPC.Basic
...
def colorBallUnion (support : Finset SixPointIndex) (center : support → Plane)
    (radius : support → ℝ) (color : SixPointColor) : Set Plane :=
...
@[simp]
theorem mem_colorBallUnion {support : Finset SixPointIndex} {center : support → Plane}
    {radius : support → ℝ} {color : SixPointColor} {x : Plane} :
...
theorem finiteBallUnion_eq_union_colorBallUnion (support : Finset SixPointIndex)
    (center : support → Plane) (radius : support → ℝ) :
...
theorem isOpen_colorBallUnion (support : Finset SixPointIndex) (center : support → Plane)
...
theorem measurableSet_colorBallUnion (support : Finset SixPointIndex)
    (center : support → Plane) (radius : support → ℝ) (color : SixPointColor) :
...
theorem measure_colorBallUnion {support : Finset SixPointIndex} (center : support → Plane)
    (radius : support → ℝ) (color : SixPointColor) (μ : Measure Plane)
...
theorem inter_colorBallUnion_subset_sdiff {support : Finset SixPointIndex}
    (center : support → Plane) (radius : support → ℝ) (e : SixPointColor → Set Plane)
...
theorem sum_measure_ball_le_union_add_leakage {support : Finset SixPointIndex}
    (center : support → Plane) (radius : support → ℝ) (e : SixPointColor → Set Plane)
    (μ : Measure Plane) (he : ∀ color, (e color).Nonempty)
...
theorem density_sum_lt_one_add_leakage_mul_ediam {support : Finset SixPointIndex}
    (hsupport : support.Nonempty) (center : support → Plane) (radius : support → ℝ)
    (e : SixPointColor → Set Plane) (μ : Measure Plane) {β scale : ℝ} {leakage : ℝ≥0∞}
