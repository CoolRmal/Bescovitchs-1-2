...
private theorem measure_rootBallUnion_sdiff_le {mu : Measure Plane} {outside : Set Plane}
    {x y : Plane} {beta gamma q d length tau : ℝ} (hq : 0 < q) (hq_one : q < 1)
...
private theorem exists_approximate_roots {e₁ e₂ : Set Plane} {d q : ℝ}
...
private theorem configuration_of_children {e : SixPointColor → Set Plane}
    (root : SixPointColor → Plane) {d gamma : ℝ} {redLeft redRight blueLeft blueRight : Plane}
...
private theorem exists_physical_configuration {mu : Measure Plane} (hmu : IsStraightMeasure mu)
    {e : SixPointColor → Set Plane} (hmeasurable : ∀ color, MeasurableSet (e color))
    ...
    (root : SixPointColor → Plane) (hroot : ∀ color, root color ∈ e color)
    (hdensity : ∀ color,
...
private theorem normalized_admissible_and_cross {physical : SixPointConfiguration}
    {e : SixPointColor → Set Plane} {s q₀ gamma d length q : ℝ} {origin : Plane}
...
private theorem SixPointPacking.ballUnionAt_inter_nonempty
    ...
    {e : SixPointColor → Set Plane} {length : ℝ} (hlength : 0 < length)
...
private theorem packing_leakage_gt {normalized physical : SixPointConfiguration}
    {e : SixPointColor → Set Plane} {mu : Measure Plane} {s beta tau margin length d scale : ℝ}
...
private theorem exists_packing_neighborhood {normalized physical : SixPointConfiguration}
    {e : SixPointColor → Set Plane} {mu : Measure Plane}
    ...
    (hdensity : ∀ x ∈ e .red ∪ e .blue, ∀ r : ℝ, 0 < r → r < scale →
      ENNReal.ofReal (2 * beta * r) < mu (Metric.ball x r)) :
    ∃ v : Set Plane, IsOpen v ∧ (v ∩ e .red).Nonempty ∧ (v ∩ e .blue).Nonempty ∧
...
private theorem exists_neighborhood_of_root_bound {e : SixPointColor → Set Plane}
    {mu : Measure Plane} {s beta q₀ gamma tau scale d length q : ℝ}
    ...
    (hd_set : d = (setEDist (e .red) (e .blue)).toReal) (root : SixPointColor → Plane)
    (hroot : ∀ color, root color ∈ e color)
...
private theorem exists_pair_neighborhood {mu : Measure Plane}
    ...
    (htau_score : tau ≤ beta * (q₀ * (beta * q₀ - s) / (4 * s * beta)) / 10)
    (hmu : IsStraightMeasure mu) {e₁ e₂ : Set Plane}
    ...
    (hdensity : ∀ x ∈ e₁ ∪ e₂, ∀ r : ℝ, 0 < r → r < scale →
      ENNReal.ofReal (2 * beta * r) < mu (Metric.ball x r)) :
    ∃ v : Set Plane, IsOpen v ∧ (v ∩ e₁).Nonempty ∧ (v ∩ e₂).Nonempty ∧
      ENNReal.ofReal tau * Metric.ediam v < mu (v \ (e₁ ∪ e₂)) := by
  let e : SixPointColor → Set Plane | .red => e₁ | .blue => e₂
  ...
  let root : SixPointColor → Plane | .red => redRoot | .blue => blueRoot
